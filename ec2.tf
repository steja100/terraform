

# Configure the AWS Provider
provider "aws" {
 version = "~> 2.0"
  region     = "ap-south-1"
  access_key = "AKIAISUVR45VCLIKFHYQ"
  secret_key = "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"
}

data aws_vpc "vpc_name"{}
data aws_security_group "workshop"{}

data aws_ami "var_ami"{
  most_recent = true

  filter {
    name   = "name"
    values = ["ubuntu/images/hvm-ssd/ubuntu-bionic-18.04-amd64-server-20191002"]
  }

  owners = ["099720109477"] # Canonical
}

resource "aws_key_pair" "sshkey" {

  key_name = "sshkey"
  public_key = "ssh-rsa "
}


resource "aws_instance" "test1" {

   ami = "${data.aws_ami.var_ami.id}"
   vpc_security_group_ids = ["${data.aws_security_group.workshop.id}"]
   instance_type = "t2.micro"
   key_name = "${aws_key_pair.sshkey.key_name}"
   tags = {
     name = "server"
   }
  
}

output "var_ami" {
  value = "${data.aws_ami.var_ami.id}"
}

output "vpc_name" {
  value = "${data.aws_vpc.vpc_name.id}"
}

output "sg_name" {
  value = "${data.aws_security_group.workshop.id}"
}

output "aws_public_ip" {
  value = "${aws_instance.test1.public_ip}"
}
